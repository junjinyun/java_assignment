...
> ui에 구현된 로그 창을 기반으로 개발자 도구를 구현하여 특정 키를 눌러서 개발자 도구를 호출(로그 창 아래에 텍스트 입력 받는 창 구현) 하여 키워드를 기반으로 명령어를 입력받아 코드 실행하는 기능 구현(ex : spawn[적, 아군 객체 소환], status[상태이상 부여])

> 생성된 객체 효과, 스텟, 정보 등을 현제 텍스트로만 존재하는 각 관련 데이터에 링크하여 데이터 구현

> 아군 스텟 정보 칸을 반 나누어 상단엔 선택한 스킬 정보(스킬버튼 그룹에서 선택된(on) 스킬), 하단엔 해당 아군 스텟표시
