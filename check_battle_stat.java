	...
	public static void CheckHp() {
		for (int y = 0; y < 8; y++) {
			if (y < 4) {// System.out.println(select_ally.ally[y].name+"의 체력 :
				...
				if (select_ally.ally[y].health <= 0 && select_ally.ally[y].alive == true) {
					select_ally.ally[y].alive = false;
				...
				if (select_enemy.enemy[y - 4].health <= 0 && select_enemy.enemy[y - 4].alive == true) {
					select_enemy.enemy[y - 4].alive = false;
