...
public class SkillButtonPanel extends JPanel implements UIObserver {
    ...
    public void updateSkillButtons() {
        ...
        for (int i = 0; i < 6; i++) {
            if (i < skills.size()) {
                ...
                // 버튼 선택 시 해당 스킬 정보를 SkillInfoPanel에 표시하고 selectedSkillNum 저장
                skillButton.addActionListener(e -> {
                    handleSkillButtonSelection(skillButton, skill, index);
                });
    ...
    private void handleSkillButtonSelection(JRadioButton selectedButton, AllySkills skill, int index) {
        ...
        // 선택된 스킬 번호를 게임플레이어에 저장하고 출력 (디버깅용)
        gamePlayer.setSelectedSkillNum(index);
        System.out.println("Selected skill index: " + index);
