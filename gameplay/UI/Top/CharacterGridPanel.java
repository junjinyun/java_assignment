	...
	public class CharacterGridPanel extends JPanel {
		...
		public CharacterGridPanel(GamePlayer gamePlayer) {
			...
			for (int i = 1; i <= 8; i++) {
				...
				CharacterUnitPanel unitPanel = new CharacterUnitPanel(mappingId, "", i, gamePlayer);
			...
			for (int i = 0; i < units.length; i++) {
				...
				if (i < 4) {
					...
					if (enemyStatus != null) {
						...
						if (!enemyStatus.getBaseStats().isAlive()) {
							// 죽었을 때만 사망 이미지
							switch (gamePlayer.getRandomStage().getId()) {
							case 1:
								setButtonIcon(cup.getButton(), "src/image/Enemy/G_dead.png");
								break;
							case 2:
								setButtonIcon(cup.getButton(), "src/image/Enemy/T_dead.png");
								break;
							case 3:
								setButtonIcon(cup.getButton(), "src/image/Enemy/A_dead.png");
								break;
							default:
								break;
							}
