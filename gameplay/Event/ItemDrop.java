...
public class ItemDrop {
    ...
    public static void runSingleDrop(GamePlayer gameplayer) {
        ...
        if (dropCashable) {
            ...
            if (random.nextDouble() < CASHABLE_DROP_RATE) {
                ...
                if (amount > 0) {
                	player.addToInventory("cashable_item", item.getId(), amount);
                	player.savePlayer("src/data/player_data.json");
            ...
            if (random.nextDouble() < USABLE_DROP_RATE) {
                int amount = getRandomAmount(item.getMinDropAmount(), item.getMaxDropAmount());
                if (amount > 0) {
                	player.addToInventory("usable_item", item.getId(), amount);
                	player.savePlayer("src/data/player_data.json");
