...
public class Battle {
	...
	public static void main(String[] args) {
		...
		System.out.println(select_enemy.enemy[0].name + " " + select_enemy.enemy[1].name + " "
				+ select_enemy.enemy[2].name + " " + select_enemy.enemy[3].name + " 이 생성되었습니다.");
		// 디버깅용
		
		System.out.println(select_ally.ally[0].name + " " + select_ally.ally[1].name + " " + select_ally.ally[2].name
				+ " " + select_ally.ally[3].name + "가 생성되었습니다.");
		// 디버깅용
		...
		while (true) {
			...
			Set_speed_act.set_speed();// 모든 캐릭터의 속도 선정 후 행동순서 선정
